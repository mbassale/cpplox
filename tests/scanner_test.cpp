...
protected:
    template <size_t N>
    void assertTokenTypes(Scanner &scanner, const std::array<TokenType, N> &expectedTokens);
};

template <size_t N>
void ScannerTest::assertTokenTypes(Scanner &scanner, const std::array<TokenType, N> &expectedTokenTypes)
{
    size_t tokenCount = 0;
    std::for_each(
        expectedTokenTypes.begin(),
