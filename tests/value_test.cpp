...
TEST(ValueTest, NullPtrAssertions) {
  ...
  EXPECT_THROW({ const auto boolValue = (bool)value; },
               std::bad_variant_access);
  EXPECT_THROW({ const auto doubleValue = (double)value; },
               std::bad_variant_access);
...
TEST(ValueTest, BoolAssertions) {
  ...
  EXPECT_THROW({ const auto nullValue = (nullptr_t)value; },
               std::bad_variant_access);
...
TEST(ValueTest, FunctionAssertions) {
  auto function = std::make_shared<Function>(FunctionType::TYPE_FUNCTION,
                                             "testFunction", 1);
  ...
  {
    const auto object2 = (ObjectPtr)value;
    // expect 4 references, one "function", one "object", one boxed in "value"
    // and "object2"
