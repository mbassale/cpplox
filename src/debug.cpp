...
VMInstr Disassembler::simpleInstruction(OpCode opCode, const std::string &name, size_t offset)
{
    return VMInstr(opCode, name, 0, offset, offset + 1);
}

VMInstr Disassembler::constantInstruction(OpCode opCode, const std::string &name, size_t offset)
{
...
{
    std::cout << std::dec << std::setfill('0') << std::setw(4) << chunk.getLines().get(offset) << " ";
}
...
void OpCodePrinter::printJumpInstruction(const VMInstr &instr)
{
    ...
    std::cout << std::hex << std::setfill('0') << std::setw(4) << (sign * jump + instr.offset + 3) << std::endl;
}